...
function showMovie(movie, formData) {
    ...
    if (!movie) {
        ...
        for (let steamingOption of movie.streamingOptions.de) {
            if (formData.streaming.includes(steamingOption.service.id)) {
                content += `<div class="watchLink"><a href="${steamingOption.link}">Watch now on ${steamingOption.service.name} <i class="fa-solid fa-film"></i></a></div>`;
                break;
            }
        }
        content += `</section>`;
